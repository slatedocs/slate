...
The [`POST /instruments/reporting`](/#settlement-equity-post-instruments-reporting) endpoint allows platforms to report their system's view on equity ownership for reconcillation purposes.

Equity ownership can be communicated to Goji as a snapshot via multiple calls to [`POST /instruments/reporting`](/#settlement-equity-post-instruments-reporting) describing investor holdings on a per-instrument basis. 

⚠️ All instruments that are part of the same snapshot should share the same `reportId`.


## Instrument Reporting Model

### Required Attributes

All of the following attributes of the Instrument Reporting Model are considered 'required' attributes. 

⚠️ In the case of an instrument that has not yet been issued or traded to investors - the holdings section can be left empty.


| Key                  | JSON Type | Value Type       | Value Description                                                                                                      |
|----------------------|-----------|------------------|------------------------------------------------------------------------------------------------------------------------|
| reportId             | String    | UUID             | A platform-generated UUID for this equity ownership snapshot, shared between all instruments included in this snapshot.|
| instrumentSymbol     | String    | InstrumentSymbol | The unique ID/symbol of the instrument registered by the platform.                                                     |
| timestamp            | String    | ISO 8601         | The time at which the snapshot of all instruments' holdings was originally taken.                                      |
...
Creates a new instrument report (part of a wider equity ownership snapshot being communicated to Goji).


### Request

Body: [Instrument Reporting Model](/#settlement-equity-instrument-reporting-model)
...
| HTTP Status Code | Description                                      | Body                          | Content-Type     |                 
|------------------|--------------------------------------------------|-------------------------------|------------------| 
| 201 Created      | Instrument report created successfully           | Instrument Reporting Model    | application/json |                 
| 400 Bad Request  | The request was malformed.  See response body    | None                          | n/a              |                  
| 401 Unauthorized | No auth provided, auth failed, or not authorized | None                          | n/a              |                 

## `GET /instruments/reporting/{reportId}/{symbol}`
...
Retrieves the details of an existing instrument's holdings at a point in time, given a equity ownership `reportId` and instrument `symbol` is supplied.

### Request

Body: [Instrument Reporting Model](/#settlement-equity-instrument-model)
...
| Code             | Description                                             | Body                       | Content-Type     |
|------------------|---------------------------------------------------------|----------------------------|------------------|
| 200 OK           | Instrument report exists and is return in the body      | Instrument Reporting Model | application/json |
| 400 Bad Request  | The request was malformed.  See response body           | None                       | n/a              |                
| 401 Unauthorized | No auth provided, auth failed, or not authorized        | None                       | n/a              |                   
