...
$api = new Api('<INSERT_API_KEY>', '<INSERT_API_SECRET>');
$success = $api->post('v2/ct/get-directories');
print_r($success);
```
...
curl -X POST \
 ...
 -d 'sig=<INSERT_API_SIG>' \
 -d 'expires=<INSERT_API_EXPIRES>' \
 https://tools.brightlocal.com/seo-tools/api/v2/ct/get-directories
```
...
var success = request.Post("/v2/ct/get-directories", parameters);
```
