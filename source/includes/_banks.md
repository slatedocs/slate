...
This guide is more like a list of building blocks and ideas for what you can do, rather than a strict step-by-step guide.
...
If you’re using the TransferWise authorization flow for new users, then you don't know if a user just signed up or is an existing user. 

[User Profiles.List](#user-profiles-list) endpoint will give you data for both personal and business profiles. This makes it easy to
figure out if a user already has a TransferWise account,or not. If the user already has a personal profile set up, then you can skip this step.

If you are using the Sign up new users via API feature then you absolutely need to create personal profile for the user.
...
## Fund transfer 
Once you have successfully created a transfer order via TransferWise API, 
you can now debit the exact source amount from your customer's bank account 
and send the funds to TransferWise’s local bank account via domestic payment.
...
### 2. Set up security for LIVE environment
  * Make sure we’ve received and successfully decrypted Live API credentials.
...
### 3. Do penny testing in LIVE
  * Launch LIVE integration to a limited set of your customers and test all currency routes end-to-end. 
