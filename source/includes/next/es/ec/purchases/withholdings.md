#### Acciones disponibles para Retenciones emitidas
...
* [`GET /purchases/withholdings`](#lista-retencións)<br>
...
totals<p class="dt-data-param-required">requerido</p> | Totales de la retención. Ver [objeto totales](#invoice-totals)
...
payments | Pagos realizados a la retención. Listado de objetos tipo [payment](#payment)
payment_methods | Listado de formas de pago aplicables a la retención. Si alguno de los métodos de pago tienen un "due_date" Datil entenderá esto como una retención a crédito. Listado de objectos tipo [payment_method](#payment-method)
...
`GET /purchases/withholdings/:invoice-id`

### Requerimiento

```shell
curl -v https://link.datil.co/withholdings/<invoice-id> \
-H "Content-Type: application/json" \
...
Reemplaza en la ruta `<invoice-ID>` por el `id` de la retención que necesitas consultar.
...
Parámetros | &nbsp;
---------- | -------
customer_tax_identification<p class="dt-data-type">string</p> | Filtra las retencións por comprador.
issue_from<p class="dt-data-type">string</p> | Lista retencións emitidas hasta esta fecha.
issue_to<p class="dt-data-type">string</p> | Lista retencións a partir de esta fecha de emisión.
sequence_from<p class="dt-data-type">string</p> | Lista retencións a partir de esta secuencia.
sequence_to<p class="dt-data-type">string</p> | Lista retencións hasta esta secuencia.
...
```json
{
  ...
  "next": "https://app.datil.co/api/v2/latest/purchases/withholdings/?page=2&issue_from=2018-02-01&page_size=2&issue_to=2018-02-10"
}
```
...
`POST /purchases/withholdings/:id/payments`

### Requerimiento


```shell
curl -v https://api.datil.co/purchases/withholdings/6463427e69b546afb77a75973cc74ce7/payments \
-H "Content-Type: application/json" \
-H "X-Api-Key: <API-key>" \
-H "X-Password: <clave-certificado-firma>" \
-d '{
...
headers = {
    ...
    'content-type': 'application/json'}
response = requests.post(
    "https://api.datil.co/purchases/withholdings/6463427e69b546afb77a75973cc74ce7/payments",
    headers = headers,
