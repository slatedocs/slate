...
max_attempts | NO | Integer | 3 | Used to specify how many times a retry should be attempted. Defaults to 5. Max allowed value is 5 times. Set to 0 to disable these configurable retries. (Note that Beyonic also does automatic retries in some cases, that are not configurable)
