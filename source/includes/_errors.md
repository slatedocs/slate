...
The Augur API uses the following error codes:
...
120 | Contract Error (details below)
...
In addition to the error codes generated by the API, there is a separate set of error codes used by Augur's [smart contracts](https://github.com/AugurProject/augur-core).  To avoid confusion, the API error codes are all positive numbers, while the contract error codes are integers less than or equal to zero.
