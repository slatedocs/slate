...
receive-email-alerts | One of 1, 2 or 0. If set to 1 we will send report alerts every time report finishes (no matter new reviews were found or not) to all email addresses specified (see field below). If you include customer email addresses when setting up your report we’ll also email them the alerts so please be sure this is what you want before adding their addresses. If set to 2 we will only send email alerts if new reviews were found upon report completion. Defaults to 0.
